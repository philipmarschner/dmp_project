...
class JointDMP:
    def __init__(self, n_bfs: int = 10, alpha: float = 48.0, beta: float = Union[float, None],
        ...
        # Scaling factor
        self.Dp = np.identity(6)

        # Initially weights are zero (no forcing term)
        self.w = np.zeros((6, self.n_bfs))
    ...
    def step(self, x, dt: float, tau: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        def fp(xj):
    ...
    def rollout(self, ts: List[float], tau: Union[float, List[float]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        self.reset()
        ...
        n_steps = len(ts)
        p = np.empty((n_steps, 6))
        dp = np.empty((n_steps, 6))
        ddp = np.empty((n_steps, 6))
