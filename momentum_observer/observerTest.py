...
demo = pandas.read_csv('./dmp_collision_500hz_slaps.csv', delimiter=',')
...
torques = target_moments
r = [observer.calcR(torques[i,:],dt,q[i,:],qd[i,:]) for i in range(6000)]
r_array = np.array(r)
...
filteredr1 = signal.medfilt(np.abs([live_lfilterx._process(y) for y in r1]),3)
filteredr2 = signal.medfilt(np.abs([live_lfiltery._process(y) for y in r2]),3)
filteredr3 = signal.medfilt(np.abs([live_lfilterz._process(y) for y in r3]),3)
filteredr4 = signal.medfilt(np.abs([live_lfilterz._process(y) for y in r4]),3)
filteredr5 = signal.medfilt(np.abs([live_lfilterz._process(y) for y in r5]),3)
filteredr6 = signal.medfilt(np.abs([live_lfilterz._process(y) for y in r6]),3)
